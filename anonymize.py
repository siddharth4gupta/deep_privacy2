...
def anonymize_image(
        image_path, input_dir: Path, output_path: Path, visualize: bool,
    ...
    out_ = np.array(im)
    md5_output = hashlib.md5(out_).hexdigest()
    if output_path is not None:
        output_path.parent.mkdir(exist_ok=True, parents=True)
        im.save(output_path, optimize=False, quality=100)
        print(f"Saved to: {output_path}")
        if md5_ == md5_output:
            file1 = open(input_dir.joinpath('non_anonymized.txt'), 'a')
            s = str(image_path.absolute())+"\n"
            file1.write(s)
            file1.close()


def anonymize_file(input_path: Path, input_dir: Path, output_path: Optional[Path], **kwargs):
    if output_path is not None and output_path.is_file():
        logger.warn(f"Overwriting previous file: {output_path}")
    if tops.is_image(input_path):
        anonymize_image(input_path, input_dir, output_path, **kwargs)
...
def anonymize_directory(og_input_dir: Path, input_dir: Path, output_dir: Path, **kwargs):
    for childname in tqdm.tqdm(input_dir.iterdir()):
        ...
        if not childpath.is_file():
            anonymize_directory(og_input_dir, childpath, output_path, **kwargs)
        else:
            assert childpath.is_file()
            anonymize_file(childpath, og_input_dir, output_path, **kwargs)
...
def anonymize_path(
    ...
    if input_path.is_dir():
        ...
        try:
            os.remove(input_path.joinpath('non_anonymized.txt'))
        except OSError:
            pass
        anonymize_directory(input_path, input_path, output_path, **kwargs)
    else:
        anonymize_file(input_path, input_path, output_path, **kwargs)
